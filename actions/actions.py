...
class ActionAskAffirmation(Action):
    ...
    async def run(self, dispatcher, tracker, domain):
        print("in ActionAskAffirmation!!!")
        story = tracker.get_slot('story')
        name = tracker.get_slot('name')
        lastOutput = tracker.latest_message['text']

        if story == 'conversation' and name is None:
            print("message is " + tracker.latest_message['text'])
            return [SlotSet("name", "リア"),FollowupAction("after_handle_did_not_understand_answer")]

        else:
            print("We are in ask affirmation action and have not handled this scenario")
            return []
